...
InventoryAddForm.propTypes = {
    ...
    onInventoryAdded: PropTypes.func
};
...
export default function InventoryAddForm({
    ...
    const [existsLocation, setExistsLocation] = useState(false);
    ...
    const handleKeyPress = async (event) => {
        ...
        if(event.which === 13){
            ...
            try {
                const response = await apiBins.getAll(`page=1&query=${getFieldProps('location_name').value}&perPage=1`);
                const bins = response.data;
                if (bins.length > 0) {
                    setLocation(bins[0]);
                    setExistsLocation(true)
    ...
    const handleChangeLocation = async (event) => {
        try {
            ...
            await setFieldValue('location_name', value);
            setExistsLocation(false)
                ...
                <RootStyle>
                    ...
                    >
                        <TextField
                            ...
                            helperText={existsLocation
                                ? `Correcta, ubicación encontrada:  ${location?.name}` : 'No encontrado '}
                            sx={{mb: 3}}
                            error={!existsLocation}
                            autoFocus
                            value={getFieldProps('location_name').value}
