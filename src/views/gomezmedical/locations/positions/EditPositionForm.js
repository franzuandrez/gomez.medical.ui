...
export default function EditPositionForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        ...
        const result = await apiPositions.patch(values, positionId);
  ...
  const { status: levelStatus } = useQuery(['position', positionId],
    async () => {

      const position = await apiPositions.getSingle(positionId);
      const sections = await apiWarehouses.nested(position.level.rack.corridor.section.warehouse.warehouse_id);
      const corridors = await apiSections.nested(position.level.rack.corridor.section.section_id);
      const racks = await apiCorridors.nested(position.level.rack.corridor.corridor_id);
      const levels = await apiRacks.nested(position.level.rack.rack_id);
      setSections(sections);
      setCorridors(corridors);
      setRacks(racks);
      setLevels(levels);

      setFieldValue('level_id', position.level.level_id, true);
      setFieldValue('rack_id', position.level.rack.rack_id, true);
      setFieldValue('corridor_id', position.level.rack.corridor.corridor_id, true);
      setFieldValue('section_id', position.level.rack.corridor.section.section_id, true);
      setFieldValue('warehouse_id', position.level.rack.corridor.section.warehouse.warehouse_id, true);
      setFieldValue('name', position.name, true);
    }
    , {
  ...
  return (
    ...
    <Page title='Posicion: Editar | Gomez-Medical'>
      <Container>
        <HeaderDashboard
          heading='Editar  Posicion'
          links={[
