...
export default function SalesOrderCreate() {
    ...
    useKeyboardShortcut(['alt','ArrowRight'], () => {

        const areProductsAdded = activeStep === 0 && cart.length > 0 && total > 0 ;
        const billingInfoComplete = activeStep === 1 && billing;
        if (areProductsAdded || billingInfoComplete) {
            handleNextStep()
        }

    }, {overrideSystem: true});
    useKeyboardShortcut(['alt','ArrowLeft'], () => {
        if (activeStep > 0) {
            handleBackStep()
        }
    }, {overrideSystem: true});
